...
void CheckFileIstFASTA(char *name){
  ...
  // THIS IS ONLY A FAST/BASIC CHECK 
  if(fgetc(F) != '>'){ 
...
CModels *CreatCModels(uint32_t nModels){
  ...
  for(x = 0 ; x < CM->nModels ; ++x){
    CM->M[x]  = CreateCModel(P->min_ctx + x, P->inverse, P->aa, P->nSym);
    CM->M[x]->id = x;
    }
...
void RemoveFiles(char *fn){

  FILE     *IN = Fopen(fn, "r");
  uint64_t indexHeader = 0, indexSymbol = 0, nFiles = 0;
  char     value;

  if(P->verbose) fprintf(stderr, "[>] Removing splitted files ...\n");
...
// for stats print. Also, counts the number of bases in each word. For AA.

uint32_t TargetAA(CModel *M, Dist *D, uint32_t id_tar){
  
  FILE     *Reader = Fopen(P->tar[id_tar], "r");
  char     *name1  = (char *) Calloc(5048, sizeof(char));
  ...
  FILE *Profile = NULL;
  if(P->profiles_r){
    name3   = (char *) Calloc(5048, sizeof(char));
    sprintf(name3, "%s-k%u.prof", P->tar[id_tar], M->ctx);
    Profile = Fopen(name3, "w");
    }
  #endif
  ...
  while((k = fread(rBuf, 1, BUFFER_SIZE, Reader))){
    for(idxPos = 0 ; idxPos < k ; ++idxPos){
      ...
      #ifdef PROGRESS
      CalcProgressTar(nSymbols, i, id_tar, P->nTar);
      #endif
      ++i;
      ...
      GetIdxAA(sBuf->buf+sBuf->idx-1, M);
      if(i > M->ctx){  
...
int32_t main(int argc, char *argv[]){
  ...
  char **p = *&argv;
  uint32_t n, x, verbose, vv, threads, nThreads, force, out, inversions, aa,
	   profiles_g, profiles_r, split, plots, min, max, nKmers, 
	   idx_reads = 0, idx_model = 0;
  ...
  aa         = ArgsState (DEF_AA,      p, argc, "-a",   "--aminoacids");
  inversions = ArgsState (DEF_IR,      p, argc, "-i",   "--ignore-ir");
  profiles_g = ArgsState (DEF_PROF_G,  p, argc, "-g",   "--ignore-gc-profiles");
  profiles_r = ArgsState (DEF_PROF_R,  p, argc, "-r",   "--ignore-raw-profiles");
  split      = ArgsState (DEF_SPLIT,   p, argc, "-s",   "--ignore-splitted-files");
  ...
  P->profiles_g = profiles_g;
  P->profiles_r = profiles_r;
  P->split      = split;
  P->nThreads   = nThreads;

  if(P->verbose) fprintf(stderr, "[>] Loading alphabet ...\n");
  AL = CreateAlphabet();
  char *str1  = CloneString(argv[argc-2]);
  char *str2  = CloneString(argv[argc-1]);
  LoadAlphabet(AL, str1, str2);
  if(P->verbose)
    PrintAlphabet(AL);
  ...
  Dist     *D = CreateDist  (nKmers, P->nSym);
  ...
  if(nThreads == 0){ // IF NO THREADS ==========================================
    ...
    for(idx_model = 0 ; idx_model < nKmers ; ++idx_model){
      ...
      if(P->aa)	    
        ...
        LoadReferenceAA(CM->M[idx_model]);
     
      PrintKHead(idx_model + min);

      double sum = 0;
      uint64_t vx[P->nTar];
      if(P->verbose) fprintf(stderr, "[>] Searching target sequences ...\n");
      for(idx_reads = 0 ; idx_reads < P->nTar ; ++idx_reads)
	if(P->aa)
          sum += (vx[idx_reads] = Target(CM->M[idx_model], D, idx_reads));
        else
          sum += (vx[idx_reads] = TargetAA(CM->M[idx_model], D, idx_reads));
  ...
  if(P->aa){
    ...
    PrintDistAAPerc(D, AL, P->min_ctx, P->plots, P->verbose);
    if(P->verbose) fprintf(stderr, "[>] Done!\n");
    }
